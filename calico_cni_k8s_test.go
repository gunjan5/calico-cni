...
var _ = Describe("CalicoCni", func() {
	...
	Describe("Run Calico CNI plugin in K8s mode", func() {
		...
		Context("using host-local IPAM", func() {
			...
			FContext("using ipAddrs annotation to assign IP address to a pod, through Calico IPAM", func() {
					...
					netconfCalicoIPAM := fmt.Sprintf(`
				{
				  ...
				  "ipam": {
					...
					ip6Pool := "2001:db8::1/120"
					testutils.CreateNewIPPool(*c, ip6Pool, false, false, true)
					_, _, err = net.ParseCIDR(ip6Pool)
					Expect(err).NotTo(HaveOccurred())
					...
					pod, err := clientset.Pods(K8S_TEST_NS).Create(&v1.Pod{
						ObjectMeta: v1.ObjectMeta{
							Name: name,
							Annotations: map[string]string{
								"cni.projectcalico.org/ipAddrs": "[\"20.0.0.111\", \"2001:db8::3\"]",
							},
						},
						Spec: v1.PodSpec{Containers: []v1.Container{{
							Name:  fmt.Sprintf("container-%s", name),
